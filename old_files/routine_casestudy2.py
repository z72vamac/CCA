...
# it = results.shape[0]
dna1 = pd.read_csv('datasets/dna_scaled2.csv').iloc[:, 1:]
# rna = pd.read_csv('datasets/rna_scaled.csv').iloc[:, 1:]
rna1 = pd.read_csv('datasets/rna_scaled2.csv').iloc[:, 1:]

genechr = np.array(pd.read_csv('datasets/genechr.csv'))

# print(genechr[[18590, 18629]])

results = pd.read_csv('results/results_chrom2.csv').iloc[:, 1:]
...
for k1, k2 in ks:
    ...
    for i in range(5):
        ...
        if total_iterations > n_rows:
            ...
            results.to_csv('./results/results_chrom2.csv')
    ...
    for i in range(5):
        ...
        if total_iterations > n_rows:
            ...
            objval_train, w1, w2, time_elapsed, n_iter = CCA(training_rna1, training_dna1, k1, k2, time_limit=tmax, best_response=True, bigM_estimation = True)
