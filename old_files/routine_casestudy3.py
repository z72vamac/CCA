...
for k1, k2 in ks:
    ...
    for i in range(cv):
        ...
        if total_iterations > n_rows:
            for j in range(10):
                training_rna1, test_rna1 = train_test_split(rna1, test_size=0.05, random_state=i)
                training_dna1, test_dna1 = train_test_split(dna1, test_size=0.05, random_state=i)
        
                objval_train, w1, w2, time_elapsed, n_iter = CCA(training_rna1, training_dna1, k1, k2, time_limit=tmax/10, init = j, best_response=True, bigM_estimation = True)
                ...
                results = results.append(serie, ignore_index=True)
                results.to_csv('./results/results_chrom1-test.csv')
    ...
    #         results = results.append(serie, ignore_index=True)
    #         results.to_csv('./results/results_chrom1_bis.csv')
