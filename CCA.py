...
def fixing_w1(w1, k2, dataset1, dataset2, time_limit=100, benders=False, bigM_estimation=True):
    ...
    if benders:
        ...
        MODEL.addConstrs(
            beta1[i] == alpha1[i] + gp.quicksum(product12[i, j] * w2[j] for j in range(p2)) - 2 * gp.quicksum(
                product11[i, j] * gamma1 * w1[i] for j in range(p1)) for i in range(p1))
        MODEL.addConstrs(
            beta2[j] == alpha2[j] + gp.quicksum(product12[i, j] * w1[i] for i in range(p1)) - 2 * gp.quicksum(
...
def CCA(dataset1, dataset2, k1, k2, time_limit=1000, init=-1, best_response=False, bigM_estimation=True):
    ...
    if best_response:
        ...
        while abs(obj_old - obj_new) >= 1e-3 and (end_time - start_time) <= time_remaining:
            ...
            obj_old = obj_new
            obj_new, w2, z2 = fixing_w1(w1, k2, dataset1, dataset2, time_limit=time_remaining / 10, benders=False,
            ...
            obj_new, w1, z1 = fixing_w2(w2, k1, dataset1, dataset2, time_limit=time_remaining / 10, benders=False,
                                        bigM_estimation=bigM_estimation)
...
def multistart_CCA(dataset1, dataset2, k1, k2, n_iter=10, time_limit=1000, best_response=False, bigM_estimation=True):
    ...
    for i in range(n_iter):
        objective, w1, w2, time_elapsed, iters = CCA(dataset1, dataset2, k1, k2, time_limit=time_limit / n_iter, init=i,
                                                     best_response=best_response, bigM_estimation=bigM_estimation)
...
def benders_CCA(dataset1, dataset2, k1, k2, time_limit=1000, init=True, bigM_estimation=True):
    ...
    if init:
        ...
        objective, w1_sol, w2_sol, time_initialization, n_iter = CCA(dataset1, dataset2, k1, k2,
                                                                     time_limit=time_limit / 10, best_response=False,
                                                                     bigM_estimation=bigM_estimation)

        if objective < 0:
            objective, w1_sol, w2_sol, time_initialization, n_iter = CCA(dataset1, dataset2, k1, k2,
                                                                         time_limit=time_limit / 10, init=0,
                                                                         best_response=False,
                                                                         bigM_estimation=bigM_estimation)
        ...
        while 1:
            ...
            obj_new, w2_sol, z2_sol, w1_sol, z1_sol, alpha1_sol, alpha2_sol, beta1_sol, beta2_sol = fixing_w1(w1_sol,
                                                                                                              k2,
                                                                                                              dataset1,
                                                                                                              dataset2,
                                                                                                              benders=True,
            ...
            obj_new, w1_sol, z1_sol, w2_sol, z2_sol, alpha1_sol, alpha2_sol, beta1_sol, beta2_sol = fixing_w2(w2_sol,
                                                                                                              k1,
                                                                                                              dataset1,
                                                                                                              dataset2,
                                                                                                              benders=True,
    ...
    while abs(UB - LB) > eps and (end_time - start_time) <= time_limit:
        objective, w1_sol, z1_sol, w2_sol, z2_sol, alpha1_sol, alpha2_sol, beta1_sol, beta2_sol = subproblem(z1_sol,
                                                                                                             z2_sol,
                                                                                                             dataset1,
                                                                                                             dataset2,
                                                                                                             k1,
...
def restricted_fixing_w1(kernel_1, bucket1, kernel_2, bucket2, objval, w1_sol, k2, dataset1, dataset2, combined=False,
                         bigM_estimation=True):
...
def restricted_fixing_w2(kernel_1, bucket1, kernel_2, bucket2, objval, w2_sol, k1, dataset1, dataset2, combined=False,
                         bigM_estimation=True):
...
def restricted_CCA(kernel_1, bucket1, kernel_2, bucket2, objval, dataset1, dataset2, k1, k2, time_limit,
    ...
    if MODEL.SolCount == 0 or MODEL.ObjVal <= 0.01:
        ...
        if best_response:
            ...
            while 1:
                ...
                obj_new, w2, z2 = restricted_fixing_w1(kernel_1, bucket1, kernel_2, bucket2, obj_old, w1_sol, k2,
                                                       dataset1, dataset2, bigM_estimation=bigM_estimation)
                ...
                obj_new, w1, z1 = restricted_fixing_w2(kernel_1, bucket1, kernel_2, bucket2, obj_old, w2, k1, dataset1,
                                                       dataset2, bigM_estimation=bigM_estimation)
...
def kernelsearch_CCA(dataset1, dataset2, k1, k2, NB, time_limit=1000, best_response=False, bigM_estimation=True):
    ...
    kernel_1, kernel_2, objective, w1, w2 = restricted_CCA(kernel_1, [], kernel_2, [], 0, dataset1, dataset2, k1, k2, t,
                                                           best_response=False, bigM_estimation=bigM_estimation)
    ...
    for bucket1, bucket2 in zip(buckets_1, buckets_2):
        ...
        kernel_1, kernel_2, objective, w1, w2 = restricted_CCA(kernel_1, bucket1, kernel_2, bucket2, max(objectives),
                                                               dataset1, dataset2, k1, k2, t,
                                                               bigM_estimation=bigM_estimation)
...
def combined_CCA(dataset1, dataset2, k1, k2, NB, time_limit=1000, bigM_estimation=True):
    ...
    while it < NB - 1:
        ...
        kernel_1, kernel_2, obj_new, w2, z2 = restricted_fixing_w1(kernel_1, [], kernel_2, bucket2, max(objectives), w1,
                                                                   k2, dataset1, dataset2, combined=True,
                                                                   bigM_estimation=bigM_estimation)
        ...
        kernel_1, kernel_2, obj_new, w1, z1 = restricted_fixing_w2(kernel_1, bucket1, kernel_2, [], max(objectives), w2,
                                                                   k1, dataset1, dataset2, combined=True,
                                                                   bigM_estimation=bigM_estimation)
...
def pareto_frontier(dataset1, dataset2, k1_min, k1_max, k2_min, k2_max, save_ws=True, time_limit=1000,
                    name_train="pareto_train", name_test="pareto_test"):
    ...
    for k1 in range(k1_min, k1_max + 1):
        for k2 in range(k2_min, k2_max + 1):
            ...
            for i in range(5):
                ...
                objval, w1, w2, time_elapsed = combined_CCA(training_dataset1, training_dataset2, k1, k2, NB=k1 * k2,
...
def initializing(mode, scaling):
    if mode == 0:

        if scaling:
            df = pd.read_csv('datasets/scaled_music.csv', sep=",")
        else:
            df = pd.read_csv('datasets/unscaled_music.csv', sep=",")
    ...
    if mode == 1:
        ...
        if scaling:
            df = pd.read_csv('datasets/scaled_winequalityred.csv', sep=",")
        else:
            df = pd.read_csv('datasets/unscaled_winequalityred.csv', sep=",")
    ...
    if mode == 2:
        ...
        if scaling:
            df = pd.read_csv('datasets/scaled_winequalitywhite.csv', sep=",")
        else:
            df = pd.read_csv('datasets/unscaled_winequalitywhite.csv', sep=",")
    ...
    if mode == 3:
        ...
        if scaling:
            df = pd.read_csv('datasets/scaled_studentpor.csv', sep=",")
        else:
            df = pd.read_csv('datasets/unscaled_studentpor.csv', sep=",")
    ...
    if mode == 4:
        ...
        if scaling:
            df = pd.read_csv('datasets/scaled_studentmat.csv', sep=",")
        else:
            df = pd.read_csv('datasets/unscaled_studentmat.csv', sep=",")
    ...
    if mode == 5:
        ...
        if scaling:
            df = pd.read_csv('datasets/scaled_yearprediction.csv', sep=",")
        else:
            df = pd.read_csv('datasets/unscaled_yearprediction.csv', sep=",")
    ...
    if mode == 6:
        ...
        if scaling:
            df = pd.read_csv('datasets/scaled_brst10a.csv', sep=",")
        else:
            df = pd.read_csv('datasets/unscaled_brst10a.csv', sep=",")

        dataset1 = df.iloc[:, 0:10]
        dataset2 = df.iloc[:, 10:20]
    ...
    if mode == 7:
        ...
        if scaling:
            df = pd.read_csv('datasets/scaled_brst10b.csv', sep=",")
        else:
            df = pd.read_csv('datasets/unscaled_brst10b.csv', sep=",")
    ...
    if mode == 8:
        ...
        if scaling:
            df = pd.read_csv('datasets/scaled_brst20a.csv', sep=",")
        else:
            df = pd.read_csv('datasets/unscaled_brst20a.csv', sep=",")

        dataset1 = df.iloc[:, 0:20]
        dataset2 = df.iloc[:, 20:40]
    ...
    if mode == 9:
        ...
        if scaling:
            df = pd.read_csv('datasets/scaled_brst20b.csv', sep=",")
        else:
            df = pd.read_csv('datasets/unscaled_brst20b.csv', sep=",")
